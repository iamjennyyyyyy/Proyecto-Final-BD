...
class PaqueteController {
  ...
  async listarTodos(req, res) {
    try {
      ...
      const datos = await paqueteService.listarTodos();
  ...
  async obtenerPorId(req, res) {
    try {
      ...
      const dato = await paqueteService.obtenerPorId(id);
  ...
  /**
    ...
    async buscarPorTermino(req, res) {
      try {
        ...
        // 3. Llamar al servicio
        const resultados = await paqueteService.buscarPorTermino(termino);
  ...
  async filtrarPorCampo(req, res) {
    try {
      ...
      const resultados = await paqueteService.filtrarPorCampo(campo, valor);
  ...
  async crear(req, res) {
    try {
      ...
      const nuevo = await paqueteService.crear(datos);
  ...
  async actualizarParcial(req, res) {
    try {
      ...
      const actualizado = await paqueteService.actualizarParcial(id, datos);
  ...
  async eliminar(req, res) {
    try {
      ...
      await paqueteService.eliminar(id);
  ...
  async mostrarPaginaListado(req, res) {
    try {
      ...
      res.render('paquete/listar', {
        titulo: 'Listado de [Paquete]',
        datos: [],
        error: error.message
      });
    }
  ...
  async mostrarFormularioEditar(req, res) {
    try {
      ...
      if (!dato) {
        ...
        return res.redirect('/paquete');
      }
      
      // const opciones = await entidadService.obtenerOpciones();
      
      res.render('paquete/editar', {
